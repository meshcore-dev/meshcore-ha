...
:::note
In [Large Mesh Mode](contacts.md#large-mesh-mode) these per-contact binary sensors are **not** created for discovered (un-added) contacts — they are tracked as data only. Added contacts keep their status sensor. The aggregate [Discovered Contact Summary sensor](contacts.md#discovered-contact-summary-sensor) provides a rollup count in either mode.
:::

...
real coordinator methods and the real sensor class; only the radio session
is mocked.
"""
import asyncio
import logging
...
async def test_fetch_stores_path_and_marks_dirty(hass: HomeAssistant):
    ...
    coordinator.api.exchange.assert_awaited_once_with("get_advert_path", FULL_PK)
...
async def test_dropped_link_does_not_count_toward_the_latch(hass: HomeAssistant):
    coordinator = _coordinator(hass)
    mock = _mock_advert_path(coordinator, None)
    mock.side_effect = RadioUnavailable("MeshCore device is not connected")

    for _ in range(4):
        await coordinator._fetch_advert_path(FULL_PK)

    assert mock.await_count == 4
    assert not coordinator._advert_path_disabled
...
async def test_overlapping_lookups_each_get_their_own_path(hass: HomeAssistant):
    """Real session and meshcore command handler, whose waiters take any ADVERT_PATH."""
    ...
    session._mesh_core = SimpleNamespace(commands=commands, is_connected=True)
...
async def test_advertisement_event_triggers_fetch(hass: HomeAssistant):
    ...
    subscribe_mock = coordinator.api.subscribe
